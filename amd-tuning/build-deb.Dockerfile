...
COPY amd-tuning amd-tuning-deps-installer install-yq.sh \
     pp-table-allowlist.txt memory-tweak-allowlist.txt \
     profile-stock.yaml profile-mi50-hbm-oc.yaml ./
...
RUN set -eux; \
    ...
    install -Dm755 install-yq.sh "${package_root}/usr/lib/amd-tuning/install-yq.sh"; \
