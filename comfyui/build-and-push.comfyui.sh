...
IMAGE_TAGS=(
  "$COMFYUI_IMAGE:${COMFYUI_BRANCH}-cuda-${COMFYUI_ROCM_VERSION}"
  "$COMFYUI_IMAGE:${COMFYUI_BRANCH}-cuda-${COMFYUI_ROCM_VERSION}-${REPO_GIT_REF}"
  "$COMFYUI_IMAGE:${COMFYUI_BRANCH}-torch-${COMFYUI_PYTORCH_VERSION}-cuda-${COMFYUI_ROCM_VERSION}-${REPO_GIT_REF}"
...
docker buildx build ${DOCKER_EXTRA_ARGS[@]} --push \
  --build-arg BASE_PYTORCH_IMAGE=${COMFYUI_TORCH_IMAGE}:${COMFYUI_PYTORCH_VERSION}-cuda${COMFYUI_ROCM_VERSION}-runtime \
